...
final class BatchExportServiceTests: XCTestCase {
    ...
    func testBatchExportUsesLocalizedFilenameWithoutChangingResultMetadata() async throws {
        ...
        XCTAssertTrue(
            result.url.lastPathComponent.hasPrefix("\(L10n.Export.filenamePrefix())_")
        )
    ...
    func testRapidBatchExportsUseUniqueLocalizedFilenames() async throws {
        ...
        async let first = BatchExportService.shared.export(
            records: records,
            format: .json,
            options: .init()
        )
        async let second = BatchExportService.shared.export(
            records: records,
            format: .json,
            options: .init()
        )
