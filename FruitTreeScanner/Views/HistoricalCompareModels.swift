...
enum HistoricalCompareSelectionPolicy {
    ...
    static func reconciled(
        ...
        let refreshedFirst = reconciledSelection(first, availableScans: availableItems)
        var refreshedSecond = reconciledSelection(second, availableScans: availableItems)
        if refreshedFirst?.id == refreshedSecond?.id {
            refreshedSecond = nil
        }
        return HistoricalCompareSelection(first: refreshedFirst, second: refreshedSecond)
    }
